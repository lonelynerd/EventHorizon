...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="styles/tt.css">
</head>
...
<div id="schedule">
    <div id="calendar">
        ...
        $tt = new Timetable("https://adecampus.univ-rouen.fr/jsp/custom/modules/plannings/anonymous_cal.jsp?resources=31782&projectId=0&calType=ical&nbWeeks=1&displayConfigId=8");
        $tt->fetchCal();
        ?>
        <div id="days">
            <?php
            for ($j = 0; $j < 6; $j += 1) {
                print_r($tt->getDay($j));
                $dateTime = $tt->getDate($tt->getDay($j));
    ...
    <div id="courses">
        ...
        for ($j = 0; $j < 6; $j += 1) {
            ...
            echo "<p class='date'>" . $tt->getFormatedDate($tt->getDay($j), "l d M") . "</p>\n";
            echo "<div class='sb'>\n";
            echo "<div class='time'>\n";
            foreach ($tt->getDay($j) as $event) {
                echo "<div class='time-se'>\n";
                echo "<p class='start'>" . $tt->getHours($event)[0] . "</p>\n";
                echo "<p class='end'>" . $tt->getHours($event)[1] . "</p>\n";
                echo "</div>\n";
            }
            echo "</div>\n";
            ...
            foreach ($tt->getDay($j) as $event) {
                ...
                if (date("U") <= $tt->getDateFromEvent($event)->format("U") && date("U") >= $tt->getDateFromEvent($event)->format("U")) {
                    echo "select-c";
                }
                echo "'>\n";
                echo "<p class='name'>" . $tt->getTitle($event) . "</p>\n";
                preg_match("/^(\w[a-zA-Z@#0-9.]* \w[a-zA-Z@#0-9.]*) \(.*\) ?([A-Z]{0,1})?.*$/", $tt->getLocation($event), $matchs);
                array_shift($matchs);
                $string = implode(" ", $matchs);
                echo "<p class='where'><img alt='location' src='./styles/location.svg'>" . $string . "</p>\n";
                echo "<p class='prof'>" . $tt->getGroup($event) . "</p>\n";
                echo "</div>\n";
            }
            echo "</div>\n";
            echo "</div>\n";
